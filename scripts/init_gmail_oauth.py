#!/usr/bin/env python3
"""Create a Gmail OAuth token for Meibook's send-only Gmail integration."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--token",
        default=os.getenv("GMAIL_TOKEN_PATH", "data/gmail_token.json"),
...
def main() -> int:
    ...
    credentials = flow.run_local_server(
        ...
        authorization_prompt_message=(
            "\nMở URL này trên trình duyệt để cấp quyền Gmail send:\n{url}\n\n"
        ),
        success_message="Đã cấp quyền Gmail. Bạn có thể đóng tab này.",
    )
    token_path.parent.mkdir(parents=True, exist_ok=True)
    token_path.write_text(credentials.to_json(), encoding="utf-8")
    os.chmod(token_path, 0o600)
    print(f"Đã lưu Gmail OAuth token vào: {token_path}")
    return 0
