...
- [ ] `config/docjp_manifest.json` category đúng theo prefix `【...】`.
- [ ] `config/research_topics.json` tồn tại và có 4 topics.
- [ ] `src/api/config.py` có config DocJP/Research topics.
- [ ] `src/api/schemas.py` có `research_topic` và response schemas mới.
- [ ] `src/api/research_topics.py` hoặc logic loader tương đương có validation topic id.
- [ ] `src/rag/vector_store.py::search()` hỗ trợ metadata filters.
- [ ] `src/rag/vector_store.py::get_session_info()` scroll phân trang và filter theo metadata.
- [ ] `src/api/main.py` tạo `docjp_vector_store` và endpoint `/research/topics`.
- [ ] `src/api/main.py` truyền `research_topic` vào RAGPipeline cho query + stream.
- [ ] `src/rag/rag_pipeline.py` có nhánh Research riêng dùng `docjp_vector_store`.
- [ ] `_resolve_model()` không hardcode Research sang Grok.
- [ ] `src/rag/prompts.py` Research prompt được cập nhật.
- [ ] `frontend/src/main.jsx` có topic selector, selected topic state, payload `research_topic`.
- [ ] `frontend/src/styles.css` có CSS responsive cho topic cards.
