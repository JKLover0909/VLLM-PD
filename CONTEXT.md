...
Meibook is MKAC's bilingual Vietnamese/Japanese internal assistant. The Production checkout provides three user-facing modes:
...
WMS and Dev's advanced report interface must not be assumed available in Production until deliberately reviewed and promoted. Dev currently permits a dedicated read-only `wms_sql_agent` to generate SQL over allowlisted WMS snapshot views when deterministic routing cannot answer or suppresses unverified semantics. If promoted, preserve the Dev provenance marker `SQL_AGENT_ANSWER_UNVERIFIED` and its read-only/allowlist/row-limit/timeout controls; do not silently reuse the MES database or MES SQL Agent.
...
- **Employee gate**: `employee_id` authorization for internal MKAC/MES access. Guest behavior is constrained and not equivalent to full employee identity.
...
- **Process**: a recorded production step/code; do not infer a name without a verified mapping.
...
- **`conversation_context`**: short client-supplied context for resolvers; not durable memory or a new data authority.
...
- MES answers must be derived from validated snapshot rows; never let an LLM fabricate operational data.
...
- Do not log or place real HR/MES PII, credentials, OAuth tokens, raw Qdrant payloads, or private source rows in tests, prompts, reports, or documentation.
...
- `src/integrations/`: MES queries, constrained SQL agent, and external actions.
