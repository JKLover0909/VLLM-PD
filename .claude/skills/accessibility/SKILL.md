...
Only run accessibility tooling that is already pinned in the project. Do not use
`npx` to download packages or run `npm install` unless the user explicitly
approves dependency changes. If no scanner is installed, perform the manual
keyboard/semantic review and report automated scanning as unavailable.

Example commands after dependencies have been reviewed and pinned:

```bash
axe http://localhost:3000 --reporter cli
...
Use a contrast checker already available in the browser or project tooling. Do
not download a CLI at runtime. Browser DevTools exposes the contrast ratio in its
Accessibility panel.
